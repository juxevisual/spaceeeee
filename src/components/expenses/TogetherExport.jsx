...
import { CATEGORY_LABELS, nowJakarta } from '../../lib/format'
...
const catMap = Object.fromEntries(Object.entries(CATEGORY_LABELS))
    ...
    <div className="flex gap-2">
      {opts.map(o => (
        <button
          ...
          style={value === o.key ? { backgroundColor: 'oklch(0.64 0.19 150)', color: 'white' } : {}}
...
function MonthYearSelect({ month, year, onMonthChange, onYearChange, years }) {
  const cls = 'px-3 py-2 text-sm rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 text-surface-900 dark:text-surface-100 focus:outline-none focus:ring-2 focus:ring-primary-500/40 [color-scheme:light] dark:[color-scheme:dark]'
  return (
    <div className="flex gap-2">
      <select value={month} onChange={e => onMonthChange(Number(e.target.value))} className={`flex-1 ${cls}`}>
...
export function TogetherExport({ onClose, month, year, filteredExpenses, familyExpenses, user, selectedPartnerId, userNames }) {
  ...
  const monthData =
    monthType === 'personal' ? filteredExpenses
    : monthType === 'family' ? familyExpenses
    : [...filteredExpenses, ...familyExpenses]
  ...
  const handleMonthExport = () => {
    downloadExpenseReport({
      expenses: monthData,
      catMap,
      title: `Together – ${MONTH_NAMES[month - 1]} ${year}`,
      fileName: `together-${year}-${pad(month)}.csv`,
      userNames,
    })
    onClose()
  }
  ...
  const handleRangeExport = async () => {
    ...
    try {
      ...
      let data
      if (rangeType === 'all') {
        const userIds = [user.id, selectedPartnerId].filter(Boolean)
        const [pRes, fRes] = await Promise.all([
          supabase.from('expenses').select('*').eq('type', 'personal').in('user_id', userIds).gte('date', fromDate).lte('date', toDate),
        ...
        const userIds = [user.id, selectedPartnerId].filter(Boolean)
        const res = await supabase.from('expenses').select('*').eq('type', 'personal').in('user_id', userIds).gte('date', fromDate).lte('date', toDate)
        data = res.data || []
      } else {
      ...
      downloadExpenseReport({
        expenses: data,
        catMap,
        title: `Together – ${titleLabel}`,
        fileName: `together-${fileSlug}.csv`,
        userNames,
      })
      onClose()
    } finally {
    ...
    >
      ...
      <div className="dialog-panel-enter relative w-full max-w-sm bg-surface-50 dark:bg-surface-900 rounded-3xl shadow-[0_24px_80px_rgba(0,0,0,0.18)] ring-1 ring-black/[0.06] dark:ring-white/[0.12] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-surface-100 dark:border-surface-800">
          <h2 className="text-sm font-bold text-surface-900 dark:text-surface-100 tracking-tight">Export Together</h2>
          <button
            onClick={onClose}
            aria-label="Close"
          ...
          <div className="inline-flex gap-0.5 p-0.5 rounded-full bg-surface-100 dark:bg-surface-800">
            {[['month', 'This month'], ['range', 'Date range']].map(([m, label]) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                style={mode === m ? { backgroundColor: 'oklch(0.64 0.19 150)' } : undefined}
          ...
          {mode === 'month' ? (
            <div className="space-y-4">
              ...
              <button
                ...
                style={{ backgroundColor: 'oklch(0.64 0.19 150)' }}
